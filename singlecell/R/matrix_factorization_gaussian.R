...
.evaluate_objective_single.gaussian <- function(dat_vec, current_vec, other_mat, scalar = 2, ...){
...
.gradient_vec.gaussian <- function(dat_vec, current_vec, other_mat, scalar = 2, ...){
  pred_vec <- other_mat %*% current_vec
  idx <- which(!is.na(dat_vec))
...
.gradient_mat.gaussian <- function(dat, pred_mat, scalar = 2, ...){
  ...
  -1/(pred_mat) - scalar^2*dat + scalar^2*dat^2*pred_mat
}
