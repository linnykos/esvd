...
test_that(".evaluate_objective yields a smaller value under truth", {
  ...
  avg_obj <- sapply(1:trials, function(x){
    ...
    for(i in 1:10){
      for(j in 1:4){
        dat_mat[i,j] <- stats::rexp(1, -pred_mat[i,j])
      }
    }
...
## .fit_exponential_factorization is correct
...
test_that(".fit_exponential_factorization preserves the positive entries", {
  trials <- 10
...
test_that(".fit_exponential_factorization can roughly recover the all 1's matrix", {
  trials <- 10
