...
generate_latent <- function(res, proportion, n){
  lapply(1:n, function(x){
    membership <- MCMCpack::rdirichlet(1, c(0.5, 0.5, 0.5))

    if(length(res$positive) > 0) positive = sum(res$positive%*%diag(as.numeric(membership))) else positive = 0
    if(length(res$negative) > 0) negative = sum(res$negative%*%diag(as.numeric(membership))) else negative = 0


    vec <- list(positive = positive, negative = negative,
                class = which.max(membership))
  })
}

inner_prod <- function(vec1, vec2){
  pos <- ifelse(length(vec1$positive) > 0, vec1$positive%*%vec2$positive, 0)
  neg <- ifelse(length(vec1$negative) > 0, vec1$negative%*%vec2$negative, 0)
  tmp <- as.numeric(sqrt(pos + neg))

  stopifnot(0 <= tmp, tmp <= 1)
