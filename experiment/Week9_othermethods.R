...
library(zinbwave)
set.seed(10)
m <- zinbFit(matrix(rpois(60, lambda=5), nrow=10, ncol=6), K = 3)
m@W
m@alpha_mu
m@alpha_pi
#
# ###
# dat <- SummarizedExperiment(matrix(rpois(60, lambda=5), nrow=10, ncol=6))
# fit <- zinbwave(dat, K = 2)
# W <- reducedDim(fit)

##########################

adj <- matrix(c(0, 0.1, 0,
                0.15,-.05,-.05,
                -.1,-.05,0.05), 3, 3, byrow = T)
tmp <- svd(adj)
...
# plot what the data really is
plot(u_dat_rescaled[,1], u_dat_rescaled[,2], col = c(1:3)[u_label], asp = T, pch = 16)
plot(v_dat_rescaled[,1], v_dat_rescaled[,2], col = c(1:3)[v_label], asp = T, pch = 16)

####################

# scale up the data
X <- floor(exp(100*dat))-1
range(X)
...
png("../figure/experiment/10_simulated_data_count.png", height = 1300, width = 2400, res = 300, units = "px")
par(mar = rep(0.5,4))
image(.rotate(X), breaks = break_vec, col = col_vec, asp = nrow(X)/ncol(X),
      axes = F)
