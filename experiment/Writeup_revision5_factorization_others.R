...
quality_vec <- sapply(1:length(res[[1]]), function(i){
  ...
  dist_mat_est <- as.matrix(stats::dist(res[[1]][[i]]$fit$fit[,1:2]))
...
# let's make that figure of the distances
...
png(paste0("../../esvd_results/figure/experiment/factorization_misspecified_density.png"),
...
png(paste0("../../esvd_results/figure/experiment/factorization_misspecified_example.png"),
    height = 1500, width = 1500, res = 300, units = "px")
label_vec <- c("SVD", "eSVD", "ZINB-WaVE", "pCMF", "UMAP", "t-SNE")
par(mfrow = c(2,3), mar = c(1, 1, 1.5, 1))
order_vec <- c(4,1,2,3,5,6)
# plot(res[[i]][[1]]$truth[,1], res[[i]][[1]]$truth[,2],
#      asp = T, pch = 16, col = col[rep(1:4, each = paramMat[1,"n_each"])],
#      xlab = "Latent dim. 1", ylab = "Latent dim. 2",
#      main = "True embedding",
#      xaxt = "n", yaxt = "n")
for(i in 1:6){
  plot(res[[i]][[1]]$fit$fit[,1], res[[i]][[1]]$fit$fit[,2],
       asp = T, pch = 16, col = col[rep(1:4, each = paramMat[1,"n_each"])],
       xlab = "Latent dim. 1", ylab = "Latent dim. 2",
       main = paste0(label_vec[i], ": (", round(res_mat[i, 1], 2), ")"),
       xaxt = "n", yaxt = "n")
}
graphics.off()
