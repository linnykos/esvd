...
.data_generator_exponential <- function(distr_func = function(x){stats::rexp(1, -x)},
                            min_val = 1e-4,
                            total = 150, col_drop = T,
                            n_each = 50){
  ...
  for(i in 1:n){
    for(j in 1:d){
      obs_mat[i,j] <- distr_func(min(gram_mat[i,j], -1e-4))
    }
  }

  # now start fidgeting with dropout
  idx <- which(gram_mat < quantile(gram_mat, probs = 0.05))
  obs_mat2 <- obs_mat
  obs_mat2[idx] <- 0 # the "true zeros"
  length(which(obs_mat2 == 0))/prod(dim(obs_mat2))
