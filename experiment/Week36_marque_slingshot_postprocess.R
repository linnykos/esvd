...
plot_lineage_2d <- function(dat, curves, name){
  ...
  png(paste0("../figure/experiment/Week36_marques_", name, "_lineage.png"),
      height = length(curves$curves)*2000/2.5, width = 2000, res = 300, units = "px")
  ...
  for(k in 1:length(curves$curves)){
    for(i in 1:ncol(combn_mat)){
      ...
      lines(curves$curves[[k]]$s[ord, idx1]*reduction_factor,
            curves$curves[[k]]$s[ord, idx2]*reduction_factor,
            lwd = 3.5, col = "white")
      lines(curves$curves[[k]]$s[ord, idx1]*reduction_factor,
            curves$curves[[k]]$s[ord, idx2]*reduction_factor,
            lwd = 3, col = "black")
...
plot_lineage_2d(u_mat, our_curves, "Our")
plot_lineage_2d(naive, naive_curves, "Naive")
