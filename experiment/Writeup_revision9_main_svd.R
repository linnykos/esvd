...
zz_pca <- stats::prcomp(svd_embedding, scale. = F, center = T)
plot(zz_pca$sdev)
...
p <- 6
set.seed(10)
svd_curves <- eSVD::slingshot(svd_embedding[,1:p], cluster_labels, starting_cluster = cluster_group_list[[1]][1],
                               cluster_group_list = cluster_group_list,
