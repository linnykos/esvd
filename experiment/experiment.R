...
# fit_factorization(dat_impute_NA, u_mat = init$u_mat, v_mat = init$v_mat,
...
if(direction == "<="){
  constr_ub <- rep(-tol, k)
  if(all(is.na(other_bound))) constr_lb <- rep(-Inf, m) else constr_lb <- rep(other_bound, m)
} else {
  constr_lb <- rep(tol, k)
  if(all(is.na(other_bound))) constr_ub <- rep(Inf, m) else constr_ub <- rep(other_bound, m)
}
