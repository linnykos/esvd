...
par(mfrow = c(2,3))
for(i in 1:6){
  plot(zz[[(i-1)*2+1]][,1], zz[[(i-1)*2+1]][,2], asp = T, pch = 16, col = col[rep(1:4, each = paramMat[1,"n_each"])])
  for(j in 1:length(zz[[(i-1)*2+2]]$curves)){ord <- zz[[(i-1)*2+2]]$curves[[j]]$ord; lines(zz[[(i-1)*2+2]]$curves[[j]]$s[ord,1], zz[[(i-1)*2+2]]$curves[[j]]$s[ord,2], lwd = 2)}
}
