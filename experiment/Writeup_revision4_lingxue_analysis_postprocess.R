...
sapply(fit_all_list, function(x){x$neg_bin_param})
sapply(fit_all_list, function(x){x$curved_gaussian_param})
...
for(dat_num in 1:length(fit_all_list)){
  ...
  main_vec[4] <- paste0(main_vec[4], ": Size=", round(fit_all_list[[dat_num]]$neg_bin_param, 2))
  main_vec[5] <- paste0(main_vec[5], ": Alpha=", round(fit_all_list[[dat_num]]$curved_gaussian_param,2))
  idx_vec <- seq(1, 9, by = 2)

  png(filename = paste0("../figure/experiment/Revision_writeup4_lingxue_embedding_", dat_num, ".png"),
...
for(dat_num in 1:length(fit_all_list)){
  ...
  png(filename = paste0("../figure/experiment/Revision_writeup4_lingxue_missing_", dat_num, ".png"),
      height = 2000, width = 2500, res = 300,
      units = "px")
  par(mfrow = c(2:3))
  for(i in 1:5){
    nat_mat <- fit_all_list[[dat_num]][[idx_vec[i]]]$u_mat %*% t(fit_all_list[[dat_num]][[idx_vec[i]]]$v_mat)
    scalar <- ifelse(family_vec[i] == "curved_gaussian", fit_all_list[[dat_num]]$curved_gaussian_param, fit_all_list[[dat_num]]$neg_bin_param)
