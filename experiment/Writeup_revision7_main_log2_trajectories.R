rm(list=ls())
load("../results/step4_factorization_log2.RData")
...
esvd_curves <- eSVD::slingshot(esvd_embedding$u_mat[,1:p], cluster_labels, starting_cluster = cluster_group_list[[1]][1],
                               ...
                               verbose = T, upscale_factor = upscale_factor, reduction_percentage = 0.2)
...
num_order_vec_esvd <- c(5, rep(3,2), c(6,1,1,4,1,4), rep(2,2),  rep(5,2))
col_vec_esvd <- color_func(1)[num_order_vec_esvd]
col_vec2_esvd <- color_func(0.5)[num_order_vec_esvd]
col_name_esvd <- c("orange", rep("bluish green", 2), c("gray", "yellow", "yellow", "blue", "yellow", "blue"), rep("skyblue", 2), rep("orange", 2))
