rm(list=ls())
load("../results/factorization_results_zinbwave_esvd.RData")
...
load("../results/factorization_results_zinbwave_rest.RData")
for(i in 1:length(res)){
  len_vec <- sapply(res[[i]], length)
  res[[i]] <- res[[i]][which(len_vec > 1)]
}
...
text(x = rep(-0.15, nrow(res_mat)), y = seq((nrow(res_mat)-1+.35)*y_spacing, 0.35*y_spacing, by = -y_spacing), labels = text_vec,
     cex = 0.9)
graphics.off()
