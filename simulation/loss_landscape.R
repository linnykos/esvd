...
png("../figure/simulation/landscape_1.png", height = 1200, width = 1200, res = 300, units = "px")
par(mar = rep(0.1,4))
plot3D::persp3D(z = .rotate(z_mat), breaks = breaks_vec, zlim = range(breaks_vec),
                colkey = F,
...
pch_vec <- c(16, 21, 16)
bg_vec <- c("black", rgb(180,200,255,maxColorValue=255), "black")
...
for(i in 1:3){
  ...
  points(colseq[col_idx], rowseq[row_idx], pch = pch_vec[i],
         bg = bg_vec[i], cex = 2.5)
}
...
legend("topleft", c("Estimate", "Population"), fill = c("black", rgb(180,200,255,maxColorValue=255)))
...
list1 <- list(fit$u_mat, fit$v_mat)
list2 <- list(res$cell_mat, res$gene_mat)
list3 <- list(fit$res_list[[4]]$u_mat, fit$res_list[[4]]$v_mat)
tmp <- .compute_directions(list1, list2, list3)
max_dist <- max(abs(c(tmp$point1 ,tmp$point2, tmp$point3)))*1.1
z_mat <- .compute_grid(dat, vec1 = tmp$vec1, dir1 = tmp$dir1, dir2 = tmp$dir2,
                       n = tmp$n, d = tmp$d, k = tmp$k,
...
png("../figure/simulation/landscape_2.png", height = 1200, width = 1200, res = 300, units = "px")
par(mar = rep(0.1,4))
plot3D::persp3D(z = .rotate(z_mat), breaks = breaks_vec, zlim = range(breaks_vec),
                colkey = F,
                xlab = "Direction 1", ylab = "Direction 2",
...
png("../figure/simulation/landscape_2b.png", height = 1200, width = 1200, res = 300, units = "px")
par(mar = c(4,4,1,1))
image(.rotate(z_mat), breaks = breaks_vec, col = jet.col(length(breaks_vec)-1),
      asp = T, axes = F, xlab = "Direction 1", ylab = "Direction 2")
...
pch_vec <- c(16, 21, 21)
bg_vec <- c("black", rgb(180,200,255,maxColorValue=255), rgb(0.5, 0.5, 0.5))
...
# #project the other points
# vec1 <- .convert_to_vector(list1[[1]], list1[[2]])
# vec2 <- .convert_to_vector(list2[[1]], list2[[2]])
# vec3 <- .convert_to_vector(list3[[1]], list3[[2]])
# tmp1 <- vec2 - vec1; tmp2 <- vec3 - vec1
# projection_mat <- tmp1 %*% solve(t(tmp1) %*% tmp1) %*% t(tmp1)
# vec_list <- vector("list", length = 5)
# for(i in 1:5){
#   zz <- .convert_to_vector(fit$res_list[[i+4]]$u_mat, fit$res_list[[i+4]]$v_mat)
#   p <- projection_mat %*% zz
#
#   #find the coordinates for this point
#   zz <- cbind(tmp$dir1, tmp$dir2)
#   p <- as.numeric(solve(t(zz)%*%zz) %*% t(zz) %*% p)
#
#   col_idx <- which.min(abs(as.numeric(colnames(z_mat)) - p[1]))
#   row_idx <- which.min(abs(as.numeric(rownames(z_mat)) - p[2]))
#   points(colseq[col_idx], rowseq[row_idx], col = "blue")
# }
...
legend("topleft", c("Estimate", "Previous estimate",
                    "Population"), fill = bg_vec[c(1,3,2)])
