...
source_code_info <- c(source_code_info, readLines("../simulation/factorization_suite_pcmf_rest.R"))
...
rule <- function(vec){
  ...
  dat <- generator_pcmf_poisson(nat_mat, dropout_prob = vec["dropout_prob"])

  list(dat = dat$dat, truth = res$cell_mat)
}
...
res <- simulation::simulation_generator(rule = rule, criterion = criterion,
                                        ...
                                        filepath = "../results/factorization_results_pcmf_rest_tmp.RData",
                                        verbose = T)

save.image("../results/factorization_results_pcmf_rest.RData")
