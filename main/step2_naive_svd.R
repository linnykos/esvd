...
svd_angle_vec <- sapply(1:nrow(paramMat_svd), function(j){
  tmp_mat <- do.call(rbind, lapply(1:cv_trials, function(i){
    ...
    set.seed(10)
    softImpute_embedding <- softImpute::softImpute(dat_NA, rank.max = paramMat_svd[j, "k"], lambda = paramMat_svd[j, "lambda"])
...
svd_missing <- lapply(1:cv_trials , function(i){
  ...
  set.seed(10)
  softImpute::softImpute(dat_NA, rank.max = paramMat_svd[idx, "k"], lambda = paramMat_svd[idx, "lambda"])
})
