...
#' @param d number of columns of intended matrix
#' @param num_val number of values to make missing in each row/column
#'
#' @return vector of indices between 1 and \code{n*d}
#' @export
construct_missing_values <- function(n, d, num_val = ceiling(min(c(4, n/10, d/10)))){
  missing_mat <- rbind(do.call(rbind, (lapply(1:n, function(x){
    cbind(x, sample(1:d, num_val))
