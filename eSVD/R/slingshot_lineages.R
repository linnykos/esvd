...
.covariance_distance <- function(mean_vec1, cov_mat1, mean_vec2, cov_mat2, n1, n2, tol = 0.1){
...
.compute_cluster_distances <- function(dat, cluster_labels){
  ...
  for(i in 1:(k-1)){
    for(j in (i+1):k){
      ...
      dist_mat[i,j] <- .covariance_distance(mean_vec1, cov_mat1, mean_vec2, cov_mat2, n1, n2)
