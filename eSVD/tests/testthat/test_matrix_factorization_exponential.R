...
## .evaluate_objective_mat.exponential is correct

test_that(".evaluate_objective_mat.exponential works", {
  set.seed(20)
  dat <- abs(matrix(rexp(40), nrow = 10, ncol = 4))
  dat[sample(1:prod(dim(dat)), 10)] <- NA
  u_mat <- abs(matrix(rnorm(20), nrow = 10, ncol = 2))
...
## .gradient_mat.exponential is correct

test_that(".gradient_mat.exponential works", {
  set.seed(20)
...
test_that(".gradient_mat.exponentialn is a proper gradient", {
  trials <- 500
