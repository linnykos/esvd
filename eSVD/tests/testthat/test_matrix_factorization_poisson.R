...
test_that(".evaluate_objective gives sensible optimal", {
  set.seed(20)
  dat <- matrix(rpois(100, lambda = exp(1/10)), nrow = 10, ncol = 10)
...
## .evaluate_objective_mat.poisson is correct

test_that(".evaluate_objective_mat.poisson works", {
  set.seed(20)
  dat <- matrix(rpois(40, lambda = 1), nrow = 10, ncol = 4)
  dat[sample(1:prod(dim(dat)), 10)] <- NA
  u_mat <- abs(matrix(rnorm(20), nrow = 10, ncol = 2))
...
## .gradient_mat.poisson is correct

test_that(".gradient_mat.poisson works", {
  set.seed(20)
  dat <- matrix(rpois(40, lambda = 1), nrow = 10, ncol = 4)
  u_mat <- abs(matrix(rnorm(20), nrow = 10, ncol = 2))
  v_mat <- abs(matrix(rnorm(8), nrow = 4, ncol = 2))
...
test_that(".gradient_mat.poissonn is a proper gradient", {
  trials <- 500
...
test_that("fit_factorization is appropriate for poisson", {
  trials <- 10
...
test_that("fit_factorization is appropriately minimized among rank 1 matrices", {
  ...
  fit_list <- lapply(1:trials, function(i){
    init <- initialization(dat_list[[i]], k = 1, family = "poisson", max_val = 100)
